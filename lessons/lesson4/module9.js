window.Lessons.lesson4.modules[8] = {
    ...
    body: `<p>You can also control height, though it can stretch the image if you are not careful.
<div class="lesson-demo-shell">
  <div class="lesson-demo-shell-header">
    <div>
      <p class="lesson-demo-eyebrow">Quick Demo</p>
      <p class="lesson-demo-heading">Height changes how tall the image stands.</p>
    </div>
    <span class="lesson-demo-chip">height tool</span>
  </div>
  <div class="lesson-demo-shell-body">
    <div class="lesson-demo-video" data-demo-video="lesson4-module9">
      <div class="lesson-demo-browser">
        <div class="lesson-demo-dots"><span></span><span></span><span></span></div>
        <div class="lesson-demo-address">height-demo.html</div>
      </div>
      <div class="lesson-demo-grid">
        <div class="lesson-demo-panel">
          <p class="lesson-demo-label">Code</p>
          <pre class="lesson-demo-snippet">&lt;img src="cat.jpg" height="300"&gt;</pre>
        </div>
        <div class="lesson-demo-panel">
          <p class="lesson-demo-label">Preview</p>
          <div class="lesson-demo-preview-frame"><div class="lesson-demo-resize"><div class="lesson-demo-size-box medium" data-size="default"></div><div class="lesson-demo-size-box tall" data-size="300px"></div></div></div>
        </div>
      </div>
      <div class="lesson-demo-captions"><div class="lesson-demo-caption active">1. Add the height attribute in pixels.</div><div class="lesson-demo-caption">2. Taller height makes the picture stretch upward.</div><div class="lesson-demo-caption">3. That is why width-only sizing is often safer.</div></div>
      <div class="lesson-demo-footer"><div class="lesson-demo-tip"><p class="lesson-demo-tip-title">Adjust</p><p class="lesson-demo-tip-copy">Use height to change the image vertically.</p></div><div class="lesson-demo-tip"><p class="lesson-demo-tip-title">Type it</p><p class="lesson-demo-tip-copy">Set the value to 300.</p></div><div class="lesson-demo-tip"><p class="lesson-demo-tip-title">Notice</p><p class="lesson-demo-tip-copy">Height can change the image shape, and the slider lets you test it live.</p></div><button type="button" class="lesson-demo-replay">Replay demo</button></div>
    </div>
  </div>
</div></p>
    <p class="text-sm italic text-gray-400 mt-4">Mission: Add <code>height="300"</code>.</p>`,
...
(function() {
  ...
  Array.from(document.querySelectorAll('.lesson-demo-video')).forEach(function(root) {
    ...
    previewScaffold: `<style>.height-lab{padding:14px;border-radius:16px;background:#0f172a;color:#e2e8f0;border:1px solid #1e3a5f;margin-bottom:14px}.height-lab input{width:100%}.height-lab img{display:block;margin:14px auto 0;transition:width .2s ease,height .2s ease;width:180px;object-fit:cover;}</style>\n<div class="height-lab">\n  <label for="height-slider-demo">Height slider</label>\n  <input id="height-slider-demo" type="range" min="80" max="340" value="180">\n  <p id="height-slider-readout">Current height: 180px</p>\n</div>\n<script>(function(){\n  const slider=document.getElementById('height-slider-demo');\n  const media=document.getElementById('height-slider-cat');\n  const readout=document.getElementById('height-slider-readout');\n  const editor=document.getElementById('code-editor');\n  const key='height-cat';\n  if(!slider||!media||!readout||!editor) return;\n  window.lesson4SliderHandlers=window.lesson4SliderHandlers||{};\n  if(window.lesson4SliderHandlers[key]) editor.removeEventListener('input', window.lesson4SliderHandlers[key]);\n  function clamp(value){\n    const number=parseInt(value,10);\n    if(Number.isNaN(number)) return 180;\n    return Math.max(80, Math.min(340, number));\n  }\n  function paint(value){\n    const number=clamp(value);\n    media.style['height']=number+'px';\n    readout.textContent='Current height: '+number+'px';\n    slider.value=number;\n  }\n  function replaceAttr(number){\n    const regex=/(<img\b[^>]*data-challenge\s*=\s*["']height-cat["'][^>]*)(>)/i;\n    let next=editor.value.replace(regex,function(match,start,end){\n      const attrRegex=new RegExp('\\bheight\\s*=\\s*["\']?\\d+["\']?','i');\n      if(attrRegex.test(start)) return start.replace(attrRegex,'height="'+number+'"')+end;\n      return start+' height="'+number+'"'+end;\n    });\n    if(next!==editor.value){\n      editor.value=next;\n      window.IntentEngine.run(window.Intents.updatePreview,{code:editor.value});\n    }\n  }\n  function syncFromCode(){\n    const match=editor.value.match(/<img\b[^>]*data-challenge\s*=\s*["']height-cat["'][^>]*\bheight\s*=\s*["']?(\d+)["']?[^>]*>/i);\n    if(match) paint(match[1]);\n  }\n  slider.addEventListener('input',function(){ paint(slider.value); replaceAttr(slider.value); });\n  window.lesson4SliderHandlers[key]=syncFromCode;\n  editor.addEventListener('input', syncFromCode);\n  syncFromCode();\n  paint(slider.value);\n})();</script>`,
