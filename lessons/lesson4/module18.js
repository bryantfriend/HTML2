window.Lessons.lesson4.modules[17] = {
    ...
    body: `<p>Videos can take up a lot of space. Width keeps the player under control.
<div class="lesson-demo-shell">
  <div class="lesson-demo-shell-header">
    <div>
      <p class="lesson-demo-eyebrow">Quick Demo</p>
      <p class="lesson-demo-heading">Width shrinks the video player to a cleaner size.</p>
    </div>
    <span class="lesson-demo-chip">video width</span>
  </div>
  <div class="lesson-demo-shell-body">
    <div class="lesson-demo-video" data-demo-video="lesson4-module18">
      <div class="lesson-demo-browser">
        <div class="lesson-demo-dots"><span></span><span></span><span></span></div>
        <div class="lesson-demo-address">video-size.html</div>
      </div>
      <div class="lesson-demo-grid">
        <div class="lesson-demo-panel">
          <p class="lesson-demo-label">Code</p>
          <pre class="lesson-demo-snippet">&lt;video src="movie.mp4" controls width="300"&gt;
&lt;/video&gt;</pre>
        </div>
        <div class="lesson-demo-panel">
          <p class="lesson-demo-label">Preview</p>
          <div class="lesson-demo-preview-frame"><div class="lesson-demo-resize"><div class="lesson-demo-size-box tall" data-size="full"></div><div class="lesson-demo-size-box medium" data-size="300px"></div></div></div>
        </div>
      </div>
      <div class="lesson-demo-captions"><div class="lesson-demo-caption active">1. Add a width attribute to the video.</div><div class="lesson-demo-caption">2. Use pixels just like with images.</div><div class="lesson-demo-caption">3. The player becomes easier to fit on the page.</div></div>
      <div class="lesson-demo-footer"><div class="lesson-demo-tip"><p class="lesson-demo-tip-title">Control</p><p class="lesson-demo-tip-copy">Resize the player with width.</p></div><div class="lesson-demo-tip"><p class="lesson-demo-tip-title">Use 300</p><p class="lesson-demo-tip-copy">Set the width exactly to 300.</p></div><div class="lesson-demo-tip"><p class="lesson-demo-tip-title">Keep neat</p><p class="lesson-demo-tip-copy">Smaller video players fit layouts better.</p></div><button type="button" class="lesson-demo-replay">Replay demo</button></div>
    </div>
  </div>
</div></p>
    <p class="text-sm italic text-gray-400 mt-4">Mission: Add <code>width="300"</code> to the video tag, then use the slider to test other widths in real time.</p>`,
...
(function() {
  ...
  Array.from(document.querySelectorAll('.lesson-demo-video')).forEach(function(root) {
    ...
    previewScaffold: `<style>.video-width-lab{padding:14px;border-radius:16px;background:#0f172a;color:#e2e8f0;border:1px solid #1e3a5f;margin-bottom:14px}.video-width-lab input{width:100%}.video-width-lab video{display:block;margin:14px auto 0;transition:width .2s ease,height .2s ease;height:auto;}</style>\n<div class="video-width-lab">\n  <label for="video-width-slider">Video width slider</label>\n  <input id="video-width-slider" type="range" min="180" max="520" value="320">\n  <p id="video-width-readout">Current width: 320px</p>\n</div>\n<script>(function(){\n  const slider=document.getElementById('video-width-slider');\n  const media=document.getElementById('video-width-player');\n  const readout=document.getElementById('video-width-readout');\n  const editor=document.getElementById('code-editor');\n  const key='video-width';\n  if(!slider||!media||!readout||!editor) return;\n  window.lesson4SliderHandlers=window.lesson4SliderHandlers||{};\n  if(window.lesson4SliderHandlers[key]) editor.removeEventListener('input', window.lesson4SliderHandlers[key]);\n  function clamp(value){\n    const number=parseInt(value,10);\n    if(Number.isNaN(number)) return 320;\n    return Math.max(180, Math.min(520, number));\n  }\n  function paint(value){\n    const number=clamp(value);\n    media.style['width']=number+'px';\n    media.setAttribute('width', number);\n    readout.textContent='Current width: '+number+'px';\n    slider.value=number;\n  }\n  function replaceAttr(number){\n    const regex=/(<video\b[^>]*data-challenge\s*=\s*["']video-width["'][^>]*)(>)/i;\n    let next=editor.value.replace(regex,function(match,start,end){\n      const attrRegex=new RegExp('\\bwidth\\s*=\\s*["\']?\\d+["\']?','i');\n      if(attrRegex.test(start)) return start.replace(attrRegex,'width="'+number+'"')+end;\n      return start+' width="'+number+'"'+end;\n    });\n    if(next!==editor.value){\n      editor.value=next;\n      window.IntentEngine.run(window.Intents.updatePreview,{code:editor.value});\n    }\n  }\n  function syncFromCode(){\n    const match=editor.value.match(/<video\b[^>]*data-challenge\s*=\s*["']video-width["'][^>]*\bwidth\s*=\s*["']?(\d+)["']?[^>]*>/i);\n    if(match) paint(match[1]);\n  }\n  slider.addEventListener('input',function(){ paint(slider.value); replaceAttr(slider.value); });\n  window.lesson4SliderHandlers[key]=syncFromCode;\n  editor.addEventListener('input', syncFromCode);\n  syncFromCode();\n  paint(slider.value);\n})();</script>`,
