window.Lessons.exam2.modules[14] = {
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
      <h3 class="heading-font text-2xl text-white">15. Which Input Type Makes a Normal Text Box?</h3>
      <span class="rounded-full border border-cyan-400/20 bg-cyan-400/10 px-3 py-1 text-[10px] font-extrabold uppercase tracking-[0.18em] text-cyan-100">Question 15 of 20</span>
    </div>
    <p class="text-[15px] leading-7 text-slate-200">Choose the input type that creates a regular typing box.</p>
...
<div class="exam2-shell" id="exam2-q15">
  <div class="exam2-top">
    <div>
      <p class="exam2-kicker">Record One Answer</p>
      <h4 class="exam2-heading">Which input type makes a normal text field?</h4>
    </div>
  ...
  <div class="exam2-option-grid cols-2"><button type="button" class="exam2-option" data-value="text" data-correct="true"><strong>text</strong><span>A regular typing field.</span></button><button type="button" class="exam2-option" data-value="checkbox" data-correct="false"><strong>checkbox</strong><span>A small check box.</span></button><button type="button" class="exam2-option" data-value="radio" data-correct="false"><strong>radio</strong><span>A choice dot.</span></button></div>
  <p class="exam2-status" data-exam2-status>Select one answer to record it.</p>
</div>
<script>

(function() {
  const root = document.getElementById('exam2-q15');
  if (!root) return;
  ...
  function recordAnswer(correct, response) {
    ...
    window.exam2Answers['q15'] = { correct: !!correct, response: response };
    if (status) status.textContent = 'Answer recorded. Continue when you are ready.';
    if (editor) {
      editor.value = appendMarker(editor.value, 'EX2_Q15_SUBMITTED');
