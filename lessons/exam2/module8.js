window.Lessons.exam2.modules[7] = {
    title: "8. What shows in the browser tab?",
  ...
  <div class="flex flex-wrap items-center justify-between gap-3">
    <h3 class="heading-font text-2xl text-white">8. What shows in the browser tab?</h3>
    <span class="rounded-full border border-cyan-400/20 bg-cyan-400/10 px-3 py-1 text-[10px] font-extrabold uppercase tracking-[0.18em] text-cyan-100">Question 8 of 50</span>
  </div>
...
<div class="exam2-shell" id="exam2-q8">
  <div class="exam2-top">
    <div>
      <p class="exam2-kicker">Record One Answer</p>
      <h4 class="exam2-heading">What shows in the browser tab?</h4>
    </div>
  ...
  <div class="exam2-option-grid"><button type="button" class="exam2-option" data-value="wrong3" data-correct="false"><strong>A. The \`alt\` text from an image.</strong><span>Tap to choose this answer.</span></button><button type="button" class="exam2-option" data-value="wrong2" data-correct="false"><strong>B. The last \`&lt;li&gt;\` tag.</strong><span>Tap to choose this answer.</span></button><button type="button" class="exam2-option" data-value="wrong1" data-correct="false"><strong>C. The first \`&lt;p&gt;\` tag.</strong><span>Tap to choose this answer.</span></button><button type="button" class="exam2-option" data-value="correct" data-correct="true"><strong>D. The text inside \`&lt;title&gt;\`.</strong><span>Tap to choose this answer.</span></button></div>
  <p class="exam2-status" data-exam2-status>Choose one answer to save it.</p>
</div>
<script>

(function() {
  const root = document.getElementById('exam2-q8');
  if (!root) return;
  ...
  function recordAnswer(correct, response) {
    ...
    window.exam2Answers['q8'] = { correct: !!correct, response: response };
    if (status) status.textContent = 'Answer saved. Move to the next question when you are ready.';
    if (editor) {
      editor.value = appendMarker(editor.value, 'EX2_Q8_SUBMITTED');
  ...
  buttons.forEach(function(button) {
    ...
    initialCode: "<!-- Question 8: click one answer on the left to save it. -->",
