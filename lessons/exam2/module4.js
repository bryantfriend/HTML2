window.Lessons.exam2.modules[3] = {
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
      <h3 class="heading-font text-2xl text-white">4. Which Tag Holds Visible Content?</h3>
      <span class="rounded-full border border-cyan-400/20 bg-cyan-400/10 px-3 py-1 text-[10px] font-extrabold uppercase tracking-[0.18em] text-cyan-100">Question 4 of 20</span>
    </div>
    <p class="text-[15px] leading-7 text-slate-200">Choose the tag that usually holds what students can actually see on the page.</p>
    <div class="grid gap-3 md:grid-cols-3">
      <div class="rounded-2xl border border-slate-400/10 bg-slate-900/60 p-3 text-sm text-slate-200"><strong class="block text-[11px] uppercase tracking-[0.18em] text-cyan-300">Format</strong>Click one answer.</div>
...
<div class="exam2-shell" id="exam2-q4">
  <div class="exam2-top">
    <div>
      <p class="exam2-kicker">Record One Answer</p>
      <h4 class="exam2-heading">Visible page content usually goes inside...</h4>
    </div>
  ...
  <p class="exam2-prompt">Pick the best answer.</p>
  <div class="exam2-option-grid cols-2"><button type="button" class="exam2-option" data-value="body" data-correct="true"><strong>&lt;body&gt;</strong><span>This holds the visible webpage content.</span></button><button type="button" class="exam2-option" data-value="head" data-correct="false"><strong>&lt;head&gt;</strong><span>This usually holds behind-the-scenes page info.</span></button><button type="button" class="exam2-option" data-value="title" data-correct="false"><strong>&lt;title&gt;</strong><span>This sets the browser tab title.</span></button></div>
  <p class="exam2-status" data-exam2-status>Select one answer to record it.</p>
</div>
<script>

(function() {
  const root = document.getElementById('exam2-q4');
  if (!root) return;
  ...
  function recordAnswer(correct, response) {
    ...
    window.exam2Answers['q4'] = { correct: !!correct, response: response };
    if (status) status.textContent = 'Answer recorded. Continue when you are ready.';
    if (editor) {
      editor.value = appendMarker(editor.value, 'EX2_Q4_SUBMITTED');
  ...
  buttons.forEach(function(button) {
    ...
    initialCode: `<!DOCTYPE html>\n<html>\n<head>\n  <meta charset="UTF-8">\n  <title>Exam 2</title>\n</head>\n<body>\n  <p>Question 4: choose one answer.</p>\n</body>\n</html>`,
