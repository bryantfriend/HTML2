window.Lessons.exam2.modules[22] = {
    title: "23. Which attribute shows play and pause buttons for audio or video?",
  ...
  <div class="flex flex-wrap items-center justify-between gap-3">
    <h3 class="heading-font text-2xl text-white">23. Which attribute shows play and pause buttons for audio or video?</h3>
    <span class="rounded-full border border-cyan-400/20 bg-cyan-400/10 px-3 py-1 text-[10px] font-extrabold uppercase tracking-[0.18em] text-cyan-100">Question 23 of 50</span>
  </div>
...
<div class="exam2-shell" id="exam2-q23">
  <div class="exam2-top">
    <div>
      <p class="exam2-kicker">Record One Answer</p>
      <h4 class="exam2-heading">Which attribute shows play and pause buttons for audio or video?</h4>
    </div>
  ...
  <div class="exam2-option-grid"><button type="button" class="exam2-option" data-value="correct" data-correct="true"><strong>A. controls</strong><span>Tap to choose this answer.</span></button><button type="button" class="exam2-option" data-value="wrong1" data-correct="false"><strong>B. autoplay</strong><span>Tap to choose this answer.</span></button><button type="button" class="exam2-option" data-value="wrong2" data-correct="false"><strong>C. loop</strong><span>Tap to choose this answer.</span></button><button type="button" class="exam2-option" data-value="wrong3" data-correct="false"><strong>D. poster</strong><span>Tap to choose this answer.</span></button></div>
  <p class="exam2-status" data-exam2-status>Choose one answer to save it.</p>
</div>
<script>

(function() {
  const root = document.getElementById('exam2-q23');
  if (!root) return;
  ...
  function recordAnswer(correct, response) {
    ...
    window.exam2Answers['q23'] = { correct: !!correct, response: response };
    if (status) status.textContent = 'Answer saved. Move to the next question when you are ready.';
    if (editor) {
      editor.value = appendMarker(editor.value, 'EX2_Q23_SUBMITTED');
  ...
  buttons.forEach(function(button) {
    ...
    initialCode: "<!-- Question 23: click one answer on the left to save it. -->",
...
#preview-area .exam-page strong { color:#0b57d0; }
    ...
    progress: 45,
    validator: function(code) { return /EX2_Q23_SUBMITTED/i.test(code); }
};
