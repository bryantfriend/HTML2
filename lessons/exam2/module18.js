window.Lessons.exam2.modules[17] = {
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
      <h3 class="heading-font text-2xl text-white">18. True or False: Header, Main, and Footer Are Semantic Tags</h3>
      <span class="rounded-full border border-cyan-400/20 bg-cyan-400/10 px-3 py-1 text-[10px] font-extrabold uppercase tracking-[0.18em] text-cyan-100">Question 18 of 20</span>
    </div>
    <p class="text-[15px] leading-7 text-slate-200">Decide whether those tags are semantic HTML.</p>
...
<div class="exam2-shell" id="exam2-q18">
  <div class="exam2-top">
    <div>
      <p class="exam2-kicker">Record One Answer</p>
      <h4 class="exam2-heading">Header, main, and footer are semantic tags.</h4>
    </div>
  ...
  <div class="exam2-option-grid cols-2"><button type="button" class="exam2-option" data-value="true" data-correct="true"><strong>True</strong><span>They describe the job of each page area.</span></button><button type="button" class="exam2-option" data-value="false" data-correct="false"><strong>False</strong><span>That statement is not correct.</span></button></div>
  <p class="exam2-status" data-exam2-status>Select one answer to record it.</p>
</div>
<script>

(function() {
  const root = document.getElementById('exam2-q18');
  if (!root) return;
  ...
  function recordAnswer(correct, response) {
    ...
    window.exam2Answers['q18'] = { correct: !!correct, response: response };
    if (status) status.textContent = 'Answer recorded. Continue when you are ready.';
    if (editor) {
      editor.value = appendMarker(editor.value, 'EX2_Q18_SUBMITTED');
