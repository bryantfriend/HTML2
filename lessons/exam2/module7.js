window.Lessons.exam2.modules[6] = {
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
      <h3 class="heading-font text-2xl text-white">7. Which Tag Makes a Bullet List?</h3>
      <span class="rounded-full border border-cyan-400/20 bg-cyan-400/10 px-3 py-1 text-[10px] font-extrabold uppercase tracking-[0.18em] text-cyan-100">Question 7 of 20</span>
    </div>
    <p class="text-[15px] leading-7 text-slate-200">Choose the list tag that makes bullet points.</p>
...
<div class="exam2-shell" id="exam2-q7">
  <div class="exam2-top">
    <div>
      <p class="exam2-kicker">Record One Answer</p>
      <h4 class="exam2-heading">A bullet list uses which outer tag?</h4>
    </div>
  ...
  <div class="exam2-option-grid cols-2"><button type="button" class="exam2-option" data-value="ul" data-correct="true"><strong>&lt;ul&gt;</strong><span>This creates an unordered bullet list.</span></button><button type="button" class="exam2-option" data-value="ol" data-correct="false"><strong>&lt;ol&gt;</strong><span>This creates a numbered list.</span></button><button type="button" class="exam2-option" data-value="li" data-correct="false"><strong>&lt;li&gt;</strong><span>This is an item inside a list.</span></button></div>
  <p class="exam2-status" data-exam2-status>Select one answer to record it.</p>
</div>
<script>

(function() {
  const root = document.getElementById('exam2-q7');
  if (!root) return;
  ...
  function recordAnswer(correct, response) {
    ...
    window.exam2Answers['q7'] = { correct: !!correct, response: response };
    if (status) status.textContent = 'Answer recorded. Continue when you are ready.';
    if (editor) {
      editor.value = appendMarker(editor.value, 'EX2_Q7_SUBMITTED');
  ...
  buttons.forEach(function(button) {
    ...
    initialCode: `<!DOCTYPE html>\n<html>\n<head>\n  <meta charset="UTF-8">\n  <title>Exam 2</title>\n</head>\n<body>\n  <p>Question 7: choose one answer.</p>\n</body>\n</html>`,
