window.Lessons.exam2.modules[11] = {
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
      <h3 class="heading-font text-2xl text-white">12. Type an Image Tag</h3>
      <span class="rounded-full border border-cyan-400/20 bg-cyan-400/10 px-3 py-1 text-[10px] font-extrabold uppercase tracking-[0.18em] text-cyan-100">Question 12 of 20</span>
    </div>
    <p class="text-[15px] leading-7 text-slate-200">Add a simple image tag using the class cat picture.</p>
    <div class="grid gap-3 md:grid-cols-3">
      <div class="rounded-2xl border border-slate-400/10 bg-slate-900/60 p-3 text-sm text-slate-200"><strong class="block text-[11px] uppercase tracking-[0.18em] text-cyan-300">Format</strong>Type the tag in the editor, then press Submit Answer.</div>
...
<div class="exam2-shell" id="exam2-q12">
  <div class="exam2-top">
    <div>
      <p class="exam2-kicker">Type Then Submit</p>
      <h4 class="exam2-heading">Type the image tag.</h4>
    </div>
  ...
  <p class="exam2-prompt">The image should use the class cat file and an alt label.</p>
  <div class="exam2-code-target">&lt;img src="assets/exam-html-cat.svg" alt="cat"&gt;</div>
...
(function() {
  const root = document.getElementById('exam2-q12');
  if (!root) return;
  ...
  function recordAnswer(correct, response) {
    ...
    window.exam2Answers['q12'] = { correct: !!correct, response: response };
    if (status) status.textContent = 'Answer recorded. Continue when you are ready.';
    if (editor) {
      editor.value = appendMarker(editor.value, 'EX2_Q12_SUBMITTED');
  ...
  if (submitButton) {
    submitButton.addEventListener('click', function() {
      ...
      const correct = (/<\\s*img\\b[^>]*\\bsrc\\s*=\\s*["\\']assets\\/exam-html-cat\\.svg["\\'][^>]*\\balt\\s*=\\s*["\\']cat["\\'][^>]*>/i.test(code) || /<\\s*img\\b[^>]*\\balt\\s*=\\s*["\\']cat["\\'][^>]*\\bsrc\\s*=\\s*["\\']assets\\/exam-html-cat\\.svg["\\'][^>]*>/i.test(code));
      recordAnswer(correct, String(code || '').trim().slice(0, 180));
    });
  }
})();
</script>`,
    initialCode: `<!DOCTYPE html>\n<html>\n<head>\n  <meta charset="UTF-8">\n  <title>Exam 2</title>\n</head>\n<body>\n  <!-- Type the image tag here -->\n\n</body>\n</html>`,
