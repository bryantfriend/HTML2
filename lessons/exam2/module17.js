window.Lessons.exam2.modules[16] = {
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
      <h3 class="heading-font text-2xl text-white">17. Which Tag Makes a Button?</h3>
      <span class="rounded-full border border-cyan-400/20 bg-cyan-400/10 px-3 py-1 text-[10px] font-extrabold uppercase tracking-[0.18em] text-cyan-100">Question 17 of 20</span>
    </div>
    <p class="text-[15px] leading-7 text-slate-200">Choose the tag that creates a clickable button.</p>
...
<div class="exam2-shell" id="exam2-q17">
  <div class="exam2-top">
    <div>
      <p class="exam2-kicker">Record One Answer</p>
      <h4 class="exam2-heading">Which tag creates a button on the page?</h4>
    </div>
  ...
  <div class="exam2-option-grid cols-2"><button type="button" class="exam2-option" data-value="button" data-correct="true"><strong>&lt;button&gt;</strong><span>This makes a clickable button.</span></button><button type="button" class="exam2-option" data-value="press" data-correct="false"><strong>&lt;press&gt;</strong><span>This is not a real HTML tag.</span></button><button type="button" class="exam2-option" data-value="click" data-correct="false"><strong>&lt;click&gt;</strong><span>This is not a real HTML tag either.</span></button></div>
  <p class="exam2-status" data-exam2-status>Select one answer to record it.</p>
</div>
<script>

(function() {
  const root = document.getElementById('exam2-q17');
  if (!root) return;
  ...
  function recordAnswer(correct, response) {
    ...
    window.exam2Answers['q17'] = { correct: !!correct, response: response };
    if (status) status.textContent = 'Answer recorded. Continue when you are ready.';
    if (editor) {
      editor.value = appendMarker(editor.value, 'EX2_Q17_SUBMITTED');
