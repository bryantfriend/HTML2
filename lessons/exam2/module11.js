window.Lessons.exam2.modules[10] = {
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
      <h3 class="heading-font text-2xl text-white">11. Which Attribute Tells the Image File Path?</h3>
      <span class="rounded-full border border-cyan-400/20 bg-cyan-400/10 px-3 py-1 text-[10px] font-extrabold uppercase tracking-[0.18em] text-cyan-100">Question 11 of 20</span>
    </div>
    <p class="text-[15px] leading-7 text-slate-200">Choose the attribute that points to the image file.</p>
...
<div class="exam2-shell" id="exam2-q11">
  <div class="exam2-top">
    <div>
      <p class="exam2-kicker">Record One Answer</p>
      <h4 class="exam2-heading">Which attribute tells the browser which image file to show?</h4>
    </div>
  ...
  <div class="exam2-option-grid cols-2"><button type="button" class="exam2-option" data-value="src" data-correct="true"><strong>src</strong><span>This points to the image file path.</span></button><button type="button" class="exam2-option" data-value="alt" data-correct="false"><strong>alt</strong><span>This describes the image with words.</span></button><button type="button" class="exam2-option" data-value="href" data-correct="false"><strong>href</strong><span>This is commonly used with links.</span></button></div>
  <p class="exam2-status" data-exam2-status>Select one answer to record it.</p>
</div>
<script>

(function() {
  const root = document.getElementById('exam2-q11');
  if (!root) return;
  ...
  function recordAnswer(correct, response) {
    ...
    window.exam2Answers['q11'] = { correct: !!correct, response: response };
    if (status) status.textContent = 'Answer recorded. Continue when you are ready.';
    if (editor) {
      editor.value = appendMarker(editor.value, 'EX2_Q11_SUBMITTED');
  ...
  buttons.forEach(function(button) {
    ...
    initialCode: `<!DOCTYPE html>\n<html>\n<head>\n  <meta charset="UTF-8">\n  <title>Exam 2</title>\n</head>\n<body>\n  <p>Question 11: choose one answer.</p>\n</body>\n</html>`,
