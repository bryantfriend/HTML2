window.Lessons.exam2.modules[12] = {
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
      <h3 class="heading-font text-2xl text-white">13. True or False: Alt Text Describes an Image</h3>
      <span class="rounded-full border border-cyan-400/20 bg-cyan-400/10 px-3 py-1 text-[10px] font-extrabold uppercase tracking-[0.18em] text-cyan-100">Question 13 of 20</span>
    </div>
    <p class="text-[15px] leading-7 text-slate-200">Decide whether alt text helps describe a picture.</p>
    <div class="grid gap-3 md:grid-cols-3">
      <div class="rounded-2xl border border-slate-400/10 bg-slate-900/60 p-3 text-sm text-slate-200"><strong class="block text-[11px] uppercase tracking-[0.18em] text-cyan-300">Format</strong>Click True or False.</div>
...
<div class="exam2-shell" id="exam2-q13">
  <div class="exam2-top">
    <div>
      <p class="exam2-kicker">Record One Answer</p>
      <h4 class="exam2-heading">Alt text describes the image for people and screen readers.</h4>
    </div>
  ...
  <p class="exam2-prompt">Pick True or False.</p>
  <div class="exam2-option-grid cols-2"><button type="button" class="exam2-option" data-value="true" data-correct="true"><strong>True</strong><span>Alt text is a description of the picture.</span></button><button type="button" class="exam2-option" data-value="false" data-correct="false"><strong>False</strong><span>That is not correct.</span></button></div>
  <p class="exam2-status" data-exam2-status>Select one answer to record it.</p>
</div>
<script>

(function() {
  const root = document.getElementById('exam2-q13');
  if (!root) return;
  ...
  function recordAnswer(correct, response) {
    ...
    window.exam2Answers['q13'] = { correct: !!correct, response: response };
    if (status) status.textContent = 'Answer recorded. Continue when you are ready.';
    if (editor) {
      editor.value = appendMarker(editor.value, 'EX2_Q13_SUBMITTED');
