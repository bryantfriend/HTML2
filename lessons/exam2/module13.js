window.Lessons.exam2.modules[12] = {
    title: "13. Label + Input Team",
    body: `<section class="quest-body">
    <p class="quest-kicker">Forms</p>
    <h3 class="quest-title">13. Label + Input Team</h3>
    <p class="quest-summary">A label tells the student what the input is for.</p>
    <div class="quest-step-grid">
      <div class="quest-step-card"><p class="quest-step-label">Watch</p><p class="quest-step-copy">The label appears above the input in the preview.</p></div>
      <div class="quest-step-card"><p class="quest-step-label">Play</p><p class="quest-step-copy">Match the same word in <code>for</code> and <code>id</code>.</p></div>
      <div class="quest-step-card"><p class="quest-step-label">Type</p><p class="quest-step-copy">Add <code>&lt;label for="name"&gt;Name&lt;/label&gt;</code> and make the input <code>id="name"</code>.</p></div>
    </div>
    <div class="quest-memory"><strong>Remember:</strong> The label <code>for</code> value should match the input <code>id</code>.</div>
    <p class="quest-mission">Mission: Connect the label and input.</p>
  </section>`,
    svg: ``,
...
<div class="quest-hero"><svg class="quest-svg-stage" viewBox="0 0 320 150" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    ...
    <rect x="28" y="26" width="264" height="18" rx="9" fill="#22d3ee"/>
    ...
    <text x="160" y="42" fill="#082f49" font-size="12" text-anchor="middle" font-family="Arial, sans-serif">Label says what to type</text>
    <text x="160" y="132" fill="#cbd5e1" font-size="12" text-anchor="middle" font-family="Arial, sans-serif">Matching words make the connection.</text>
  </svg></div>
<div class="quest-widget-shell quest-sequence-shell"
      data-quest-sequence="true"
      data-marker="EX2_M13_FLOW"
      data-success="Nice. Labels and inputs work best together."
    ...
    <div class="quest-widget-top">
      <div>
        ...
        <h4 class="quest-widget-title">Build the label team in order.</h4>
      </div>
      <span class="quest-chip">2 steps</span>
    </div>
    <div class="quest-sequence-track">
      <div class="quest-sequence-node is-current" data-node="label">1</div><div class="quest-sequence-node" data-node="input">2</div>
    </div>
    <div class="quest-sequence-buttons">
      <button type="button" class="quest-sequence-btn" data-value="label">Place the label first</button><button type="button" class="quest-sequence-btn" data-value="input">Then match the input id</button>
    </div>
    <p class="quest-status" id="exam2-13-status">Tap the buttons in the correct order.</p>
  </div>
<script>
(function() {
  ...
  document.querySelectorAll('[data-quest-toggle="true"]').forEach(function(root) {
    ...
    progress: 65,
    validator: function(code) { return code.includes("<!-- EX2_M13_FLOW -->") && /<\s*label\b[^>]*>[\s\S]*?name[\s\S]*?<\s*\/\s*label\s*>/i.test(code) && /\bfor\s*=\s*["']name["']/i.test(code) && /\bid\s*=\s*["']name["']/i.test(code); }
};