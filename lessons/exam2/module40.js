window.Lessons.exam2.modules[39] = {
    title: "40. What does the `method` attribute do on a form?",
  ...
  <div class="flex flex-wrap items-center justify-between gap-3">
    <h3 class="heading-font text-2xl text-white">40. What does the \`method\` attribute do on a form?</h3>
    <span class="rounded-full border border-cyan-400/20 bg-cyan-400/10 px-3 py-1 text-[10px] font-extrabold uppercase tracking-[0.18em] text-cyan-100">Question 40 of 50</span>
  </div>
...
<div class="exam2-shell" id="exam2-q40">
  <div class="exam2-top">
    <div>
      <p class="exam2-kicker">Record One Answer</p>
      <h4 class="exam2-heading">What does the \`method\` attribute do on a form?</h4>
    </div>
...
(function() {
  const root = document.getElementById('exam2-q40');
  if (!root) return;
  ...
  function recordAnswer(correct, response) {
    ...
    window.exam2Answers['q40'] = { correct: !!correct, response: response };
    if (status) status.textContent = 'Answer saved. Move to the next question when you are ready.';
    if (editor) {
      editor.value = appendMarker(editor.value, 'EX2_Q40_SUBMITTED');
  ...
  buttons.forEach(function(button) {
    ...
    initialCode: "<!-- Question 40: click one answer on the left to save it. -->",
...
#preview-area .exam-page strong { color:#0b57d0; }
    ...
    progress: 78,
    validator: function(code) { return /EX2_Q40_SUBMITTED/i.test(code); }
};
