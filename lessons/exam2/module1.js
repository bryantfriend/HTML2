window.Lessons.exam2.modules[0] = {
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
      <h3 class="heading-font text-2xl text-white">1. What Is HTML Mostly Used For?</h3>
      <span class="rounded-full border border-cyan-400/20 bg-cyan-400/10 px-3 py-1 text-[10px] font-extrabold uppercase tracking-[0.18em] text-cyan-100">Question 1 of 20</span>
    </div>
    <p class="text-[15px] leading-7 text-slate-200">Choose the best job for HTML.</p>
...
<div class="exam2-shell" id="exam2-q1">
  <div class="exam2-top">
    <div>
      <p class="exam2-kicker">Record One Answer</p>
      <h4 class="exam2-heading">HTML mainly helps a webpage with...</h4>
    </div>
  ...
  <div class="exam2-option-grid cols-2"><button type="button" class="exam2-option" data-value="structure" data-correct="true"><strong>Structure</strong><span>It builds the parts of the page.</span></button><button type="button" class="exam2-option" data-value="color" data-correct="false"><strong>Color</strong><span>That is mostly CSS.</span></button><button type="button" class="exam2-option" data-value="animation" data-correct="false"><strong>Animation</strong><span>That is usually JavaScript.</span></button></div>
  <p class="exam2-status" data-exam2-status>Select one answer to record it.</p>
</div>
<script>

(function() {
  const root = document.getElementById('exam2-q1');
  if (!root) return;
  ...
  function recordAnswer(correct, response) {
    ...
    window.exam2Answers['q1'] = { correct: !!correct, response: response };
    if (status) status.textContent = 'Answer recorded. Continue when you are ready.';
    if (editor) {
      editor.value = appendMarker(editor.value, 'EX2_Q1_SUBMITTED');
  ...
  buttons.forEach(function(button) {
    ...
    initialCode: `<!DOCTYPE html>\n<html>\n<head>\n  <meta charset="UTF-8">\n  <title>Exam 2</title>\n</head>\n<body>\n  <p>Question 1: click one answer on the left.</p>\n</body>\n</html>`,
