window.Lessons.exam2.modules[18] = {
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
      <h3 class="heading-font text-2xl text-white">19. Which Tag Usually Holds Menu Links?</h3>
      <span class="rounded-full border border-cyan-400/20 bg-cyan-400/10 px-3 py-1 text-[10px] font-extrabold uppercase tracking-[0.18em] text-cyan-100">Question 19 of 20</span>
    </div>
    <p class="text-[15px] leading-7 text-slate-200">Choose the semantic tag commonly used for navigation links.</p>
...
<div class="exam2-shell" id="exam2-q19">
  <div class="exam2-top">
    <div>
      <p class="exam2-kicker">Record One Answer</p>
      <h4 class="exam2-heading">Which semantic tag usually holds menu links?</h4>
    </div>
  ...
  <div class="exam2-option-grid cols-2"><button type="button" class="exam2-option" data-value="nav" data-correct="true"><strong>&lt;nav&gt;</strong><span>This is commonly used for navigation menus.</span></button><button type="button" class="exam2-option" data-value="article" data-correct="false"><strong>&lt;article&gt;</strong><span>This is commonly used for one story or post.</span></button><button type="button" class="exam2-option" data-value="footer" data-correct="false"><strong>&lt;footer&gt;</strong><span>This is usually the bottom section.</span></button></div>
  <p class="exam2-status" data-exam2-status>Select one answer to record it.</p>
</div>
<script>

(function() {
  const root = document.getElementById('exam2-q19');
  if (!root) return;
  ...
  function recordAnswer(correct, response) {
    ...
    window.exam2Answers['q19'] = { correct: !!correct, response: response };
    if (status) status.textContent = 'Answer recorded. Continue when you are ready.';
    if (editor) {
      editor.value = appendMarker(editor.value, 'EX2_Q19_SUBMITTED');
