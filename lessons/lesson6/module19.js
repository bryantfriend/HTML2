window.Lessons.lesson6.modules[18] = {
    ...
    <h3 class="quest-title">19. Group the Table</h3>
    <p class="quest-summary">Big tables can group rows with <code>&lt;thead&gt;</code> and <code>&lt;tbody&gt;</code>.</p>
    <div class="quest-step-grid">
      <div class="quest-step-card"><p class="quest-step-label">Watch</p><p class="quest-step-copy">The header row and the data rows get their own groups.</p></div>
      <div class="quest-step-card"><p class="quest-step-label">Play</p><p class="quest-step-copy">Tap the grouping order in the right sequence.</p></div>
      <div class="quest-step-card"><p class="quest-step-label">Type</p><p class="quest-step-copy">Wrap the first row in <code>&lt;thead&gt;</code> and the second row in <code>&lt;tbody&gt;</code>.</p></div>
    </div>
    <div class="quest-memory"><strong>Remember:</strong> thead = labels, tbody = regular rows.</div>
    <p class="quest-mission">Mission: Group the table rows.</p>
  </section>`,
    svg: ``,
...
<div class="quest-hero"><svg class="quest-svg-stage" viewBox="0 0 320 150" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    ...
    <text x="270" y="42" fill="#e0f2fe" font-size="12" font-family="Arial, sans-serif" text-anchor="end">Group the grid</text>
  </svg></div>
<div class="quest-widget-shell quest-sequence-shell"
      data-quest-sequence="true"
      data-marker="TABLE_GROUP_READY"
      data-success="Great. The table groups are in the right order."
      data-reset="Start at step 1 again."
      data-insertions=""
      data-order="thead|tbody">
    <div class="quest-widget-top">
      <div>
        <p class="quest-widget-kicker">Mini Game</p>
        <h4 class="quest-widget-title">Group the rows in order.</h4>
      </div>
      <span class="quest-chip">2 steps</span>
    </div>
    <div class="quest-sequence-track">
      <div class="quest-sequence-node is-current" data-node="thead">1</div><div class="quest-sequence-node" data-node="tbody">2</div>
    </div>
    <div class="quest-sequence-buttons">
      <button type="button" class="quest-sequence-btn" data-value="thead">Group the heading row</button><button type="button" class="quest-sequence-btn" data-value="tbody">Group the data rows</button>
    </div>
    <p class="quest-status" id="lesson6-19-status">Tap the buttons in the correct order.</p>
  </div>
<script>
(function() {
  ...
  document.querySelectorAll('[data-quest-toggle="true"]').forEach(function(root) {
    ...
    progress: 95,
    validator: function(code) { return code.includes("<!-- TABLE_GROUP_READY -->") && /<\s*thead\b/i.test(code) && /<\s*\/\s*thead\s*>/i.test(code) && /<\s*tbody\b/i.test(code) && /<\s*\/\s*tbody\s*>/i.test(code); }
};