window.Lessons.lesson5.modules[14] = {
    ...
    <h3 class="quest-title">15. Dropdown Menus</h3>
    <p class="quest-summary">A <code>&lt;select&gt;</code> tag creates a dropdown menu, and each choice lives in an <code>&lt;option&gt;</code> tag.</p>
    <div class="quest-step-grid">
      <div class="quest-step-card"><p class="quest-step-label">Watch</p><p class="quest-step-copy">The select holds the option choices.</p></div>
      <div class="quest-step-card"><p class="quest-step-label">Play</p><p class="quest-step-copy">See the closed menu and imagine the choices inside it.</p></div>
      <div class="quest-step-card"><p class="quest-step-label">Type</p><p class="quest-step-copy">Add a <code>&lt;select&gt;</code> with one <code>&lt;option&gt;</code>.</p></div>
    </div>
    <div class="quest-memory"><strong>Remember:</strong> Select on the outside. Option on the inside.</div>
    <p class="quest-mission">Mission: Create a dropdown menu with one option.</p>
  </section>`,
    svg: ``,
...
<div class="quest-hero"><svg class="quest-svg-stage" viewBox="0 0 320 140" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    <defs>
      <linearGradient id="form-glow-dropdown" x1="0%" x2="100%" y1="0%" y2="100%">
    ...
    <rect x="44" y="44" width="100" height="12" rx="6" fill="url(#form-glow-dropdown)" opacity="0.95"/>
    <rect x="44" y="68" width="232" height="16" rx="8" fill="#e0f2fe" opacity="0.92"/>
    <rect x="44" y="92" width="190" height="14" rx="7" fill="url(#form-glow-dropdown)"/>
    <text x="242" y="48" fill="#bae6fd" font-size="12" font-family="Arial, sans-serif" text-anchor="end">Dropdown</text>
  </svg></div>
<div class="quest-widget-shell">
    <div class="quest-widget-top">
      <div>
        <p class="quest-widget-kicker">Quick Demo</p>
        <h4 class="quest-widget-title">A dropdown stores its choices inside option tags.</h4>
      </div>
      <span class="quest-chip">select</span>
    </div>
    <div class="quest-demo" data-quest-demo="true">
      <div class="quest-browser">
        ...
        <div class="quest-browser-bar">menu-form.html</div>
      </div>
      <div class="quest-demo-grid">
        <div class="quest-demo-panel">
          <p class="quest-panel-label">Code</p>
          <pre class="quest-demo-snippet">&lt;select&gt;
  &lt;option&gt;Cool&lt;/option&gt;
&lt;/select&gt;</pre>
        </div>
        <div class="quest-demo-panel">
          <p class="quest-panel-label">Preview</p>
          <div class="quest-demo-preview"><div style="width:100%;max-width:250px;padding:14px;border-radius:20px;background:linear-gradient(180deg,#f8fbff,#e0f2fe);box-shadow:inset 0 0 0 1px rgba(96,165,250,0.18);"><div style="display:grid;gap:6px;margin-bottom:10px;"><div style="font:700 12px/1.2 sans-serif;color:#0f172a;letter-spacing:0.04em;">Favorite snack</div><div style="padding:10px 12px;border-radius:12px;background:white;border:1px solid #93c5fd;color:#0f172a;font:700 13px/1.2 sans-serif;display:flex;justify-content:space-between;"><span>Cool</span><span>v</span></div></div></div></div>
        </div>
      </div>
      <div class="quest-caption-stack">
        <p class="quest-caption is-active">Open the select tag.</p><p class="quest-caption">Place option tags inside it.</p><p class="quest-caption">Now build your own dropdown in the editor.</p>
      </div>
...
(function() {
  ...
  document.querySelectorAll('[data-quest-toggle="true"]').forEach(function(root) {
    ...
    progress: 75,
    validator: function(code) { return /<\s*select\b/i.test(code) && /<\s*\/\s*select\s*>/i.test(code) && (code.match(/<\s*option\b/gi) || []).length >= 1; }
};