window.Lessons.lesson5.modules[9] = {
    ...
    <h3 class="quest-title">10. Link Label + Input</h3>
    <p class="quest-summary">A label can point to the exact input by using <code>for</code> on the label and <code>id</code> on the input.</p>
    <div class="quest-step-grid">
      <div class="quest-step-card"><p class="quest-step-label">Watch</p><p class="quest-step-copy">Both names must match exactly.</p></div>
      <div class="quest-step-card"><p class="quest-step-label">Play</p><p class="quest-step-copy">Look closely at the demo: the label gets <code>for="user"</code> and the input gets <code>id="user"</code>. Those two words must match.</p></div>
      <div class="quest-step-card"><p class="quest-step-label">Type</p><p class="quest-step-copy">You must add BOTH pieces. Step 1: add <code>for="user"</code> inside the opening <code>&lt;label&gt;</code> tag. Step 2: add <code>id="user"</code> inside the opening <code>&lt;input&gt;</code> tag. The mission only completes when both appear.</p></div>
    </div>
    <div class="quest-memory"><strong>Remember:</strong> The matching words must be identical, and both attributes are required.</div>
    <p class="quest-mission">Mission: Add <code>for="user"</code> to the label and <code>id="user"</code> to the input so clicking the label targets the correct box.</p>
  </section>`,
    svg: ``,
...
<div class="quest-hero"><svg class="quest-svg-stage" viewBox="0 0 320 140" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    <defs>
      <linearGradient id="form-glow-match-the-names" x1="0%" x2="100%" y1="0%" y2="100%">
    ...
    <rect x="44" y="44" width="100" height="12" rx="6" fill="url(#form-glow-match-the-names)" opacity="0.95"/>
    <rect x="44" y="68" width="232" height="16" rx="8" fill="#e0f2fe" opacity="0.92"/>
    <rect x="44" y="92" width="168" height="14" rx="7" fill="url(#form-glow-match-the-names)"/>
    <text x="242" y="48" fill="#bae6fd" font-size="12" font-family="Arial, sans-serif" text-anchor="end">Match the names</text>
  </svg></div>
<div class="quest-widget-shell">
    <div class="quest-widget-top">
      <div>
        <p class="quest-widget-kicker">Quick Demo</p>
        <h4 class="quest-widget-title">Matching names create a link.</h4>
      </div>
      <span class="quest-chip">for + id</span>
    </div>
...
(function() {
  ...
  document.querySelectorAll('[data-quest-toggle="true"]').forEach(function(root) {
    ...
    progress: 50,
    validator: function(code) { return new RegExp("<\\s*label\\b[^>]*\\bfor\\s*=\\s*(?:['\"]user['\"]|user)", 'i').test(code) && new RegExp("<\\s*input\\b[^>]*\\bid\\s*=\\s*(?:['\"]user['\"]|user)", 'i').test(code); }
};