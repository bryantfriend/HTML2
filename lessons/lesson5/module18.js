window.Lessons.lesson5.modules[17] = {
    ...
    <h3 class="quest-title">18. Method = How It Travels</h3>
    <p class="quest-summary">The <code>method</code> tells the browser how to send the data. For passwords, we usually use POST.</p>
    <div class="quest-step-grid">
      <div class="quest-step-card"><p class="quest-step-label">Watch</p><p class="quest-step-copy">GET puts data in the URL. POST sends it more safely for login forms.</p></div>
      <div class="quest-step-card"><p class="quest-step-label">Play</p><p class="quest-step-copy">Choose the better method for a password form.</p></div>
      <div class="quest-step-card"><p class="quest-step-label">Type</p><p class="quest-step-copy">Add <code>method="POST"</code> to the form.</p></div>
    </div>
    <div class="quest-memory"><strong>Remember:</strong> For login forms, POST is the better choice.</div>
    <p class="quest-mission">Mission: Make the form use POST.</p>
  </section>`,
    svg: ``,
...
<div class="quest-hero"><svg class="quest-svg-stage" viewBox="0 0 320 140" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    <defs>
      <linearGradient id="form-glow-send-with-post" x1="0%" x2="100%" y1="0%" y2="100%">
    ...
    <rect x="44" y="44" width="100" height="12" rx="6" fill="url(#form-glow-send-with-post)" opacity="0.95"/>
    <rect x="44" y="68" width="232" height="16" rx="8" fill="#e0f2fe" opacity="0.92"/>
    <rect x="44" y="92" width="182" height="14" rx="7" fill="url(#form-glow-send-with-post)"/>
    <text x="242" y="48" fill="#bae6fd" font-size="12" font-family="Arial, sans-serif" text-anchor="end">Send with POST</text>
  </svg></div>
<div class="quest-widget-shell quest-choice-shell"
      data-quest-choice="true"
      data-marker="METHOD_READY"
      data-success="Correct. POST is the right choice here."
    ...
    <div class="quest-widget-top">
      <div>
        ...
        <h4 class="quest-widget-title">Which method fits a password form?</h4>
      </div>
      <span class="quest-chip">one answer</span>
    </div>
    <div class="quest-choice-scene"></div>
    <p class="quest-choice-prompt">Tap the better method for a login form.</p>
    <div class="quest-choice-grid quest-choice-grid-2">
      <button type="button" class="quest-choice-btn" data-value="post"><span>POST</span><small>Used for login and form data.</small></button><button type="button" class="quest-choice-btn" data-value="get"><span>GET</span><small>Often shows data in the URL.</small></button>
    </div>
    <p class="quest-status" id="lesson5-18-status">Tap the best answer.</p>
  </div>
<script>
(function() {
  ...
  document.querySelectorAll('[data-quest-toggle="true"]').forEach(function(root) {
    ...
    progress: 90,
    validator: function(code) { return code.includes("<!-- METHOD_READY -->") && new RegExp("<\\s*form\\b[^>]*\\bmethod\\s*=\\s*(?:['\"]POST['\"]|POST)", 'i').test(code); }
};