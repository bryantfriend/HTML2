window.Lessons.lesson5.modules[5] = {
    ...
    <h3 class="quest-title">6. Checkbox Mission</h3>
    <p class="quest-summary">Checkboxes are for choices where you can pick more than one thing.</p>
    <div class="quest-step-grid">
      <div class="quest-step-card"><p class="quest-step-label">Watch</p><p class="quest-step-copy">Look for the choices that can all be true at the same time.</p></div>
      <div class="quest-step-card"><p class="quest-step-label">Play</p><p class="quest-step-copy">Tap every card that could use a checkbox.</p></div>
      <div class="quest-step-card"><p class="quest-step-label">Type</p><p class="quest-step-copy">Then type <code>&lt;input type="checkbox"&gt;</code> in the editor.<br><code>&lt;input type="checkbox"&gt;</code></p></div>
    </div>
    <div class="quest-memory"><strong>Remember:</strong> Checkbox = many answers can be on.</div>
    <p class="quest-mission">Mission: Solve the mini game, then add a checkbox input.</p>
  </section>`,
    svg: ``,
...
<div class="quest-hero"><svg class="quest-svg-stage" viewBox="0 0 320 140" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    <defs>
      <linearGradient id="form-glow-many-choices" x1="0%" x2="100%" y1="0%" y2="100%">
    ...
    <rect x="44" y="44" width="100" height="12" rx="6" fill="url(#form-glow-many-choices)" opacity="0.95"/>
    <rect x="44" y="68" width="232" height="16" rx="8" fill="#e0f2fe" opacity="0.92"/>
    <rect x="44" y="92" width="188" height="14" rx="7" fill="url(#form-glow-many-choices)"/>
    <text x="242" y="48" fill="#bae6fd" font-size="12" font-family="Arial, sans-serif" text-anchor="end">Many choices</text>
  </svg></div>
<div class="quest-widget-shell quest-choice-shell"
      data-quest-choice="true"
      data-marker="CHECKBOX_READY"
      data-success="Perfect. Those are checkbox style choices."
      data-retry="Nice try. Pick the card that matches the mission."
      data-correct="pizza|club|skills"
      data-multi="true">
    <div class="quest-widget-top">
      <div>
        <p class="quest-widget-kicker">Mini Game</p>
        <h4 class="quest-widget-title">Find the checkbox jobs.</h4>
      </div>
      <span class="quest-chip">3 correct</span>
    </div>
    <div class="quest-choice-scene"></div>
    <p class="quest-choice-prompt">Which choices could all be selected at once?</p>
    <div class="quest-choice-grid quest-choice-grid-2">
      <button type="button" class="quest-choice-btn" data-value="pizza"><span>Pizza toppings</span><small>You can choose cheese, olives, and peppers together.</small></button><button type="button" class="quest-choice-btn" data-value="club"><span>School clubs</span><small>You might join coding and art.</small></button><button type="button" class="quest-choice-btn" data-value="pets"><span>Favorite pet</span><small>Usually one answer in a poll.</small></button><button type="button" class="quest-choice-btn" data-value="skills"><span>Skills you have</span><small>You can have several skills.</small></button><button type="button" class="quest-choice-btn" data-value="winner"><span>Race winner</span><small>There is only one winner.</small></button>
    </div>
    <p class="quest-status" id="lesson5-6-status">Find every correct card.</p>
  </div>
<script>
(function() {
  ...
  document.querySelectorAll('[data-quest-toggle="true"]').forEach(function(root) {
    ...
    progress: 30,
    validator: function(code) { return code.includes("<!-- CHECKBOX_READY -->") && new RegExp("<\\s*input\\b[^>]*\\btype\\s*=\\s*(?:['\"]checkbox['\"]|checkbox)", 'i').test(code); }
};