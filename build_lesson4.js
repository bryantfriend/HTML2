...
const modules = [
  ...require('./scripts/lesson4_module_data_part1'),
  ...require('./scripts/lesson4_module_data_part2')
];
...
const sharedWidgetCode = `<!-- INTERACTIVE MODULE -->
<style>
...
function escTemplate(value) { return String(value || '').replace(/`/g, '\\`'); }
function escDouble(value) { return String(value || '').replace(/\\/g, '\\\\').replace(/"/g, '\\"'); }
function renderTips(tips) {
  return tips.map((tip) => `<div class="lesson-demo-tip"><p class="lesson-demo-tip-title">${tip.title}</p><p class="lesson-demo-tip-copy">${tip.copy}</p></div>`).join('');
}
function renderCaptions(captions) {
  return captions.map((caption, index) => `<div class="lesson-demo-caption${index === 0 ? ' active' : ''}">${caption}</div>`).join('');
}
function buildBody(mod, index) {
  return `${mod.intro}
<div class="lesson-demo-shell">
  <div class="lesson-demo-shell-header">
    <div>
      <p class="lesson-demo-eyebrow">Quick Demo</p>
      <p class="lesson-demo-heading">${mod.demoHeading}</p>
    </div>
    <span class="lesson-demo-chip">${mod.chip}</span>
  </div>
  <div class="lesson-demo-shell-body">
    <div class="lesson-demo-video" data-demo-video="lesson4-module${index + 1}">
      <div class="lesson-demo-browser">
        <div class="lesson-demo-dots"><span></span><span></span><span></span></div>
        <div class="lesson-demo-address">${mod.browserTitle}</div>
      </div>
      <div class="lesson-demo-grid">
        <div class="lesson-demo-panel">
          <p class="lesson-demo-label">Code</p>
          <pre class="lesson-demo-snippet">${mod.demoCode}</pre>
        </div>
        <div class="lesson-demo-panel">
          <p class="lesson-demo-label">Preview</p>
          <div class="lesson-demo-preview-frame">${mod.demoPreview}</div>
        </div>
      </div>
      <div class="lesson-demo-captions">${renderCaptions(mod.captions)}</div>
      <div class="lesson-demo-footer">${renderTips(mod.tips)}<button type="button" class="lesson-demo-replay">Replay demo</button></div>
    </div>
  </div>
</div>`;
}
...
modules.forEach((mod, index) => {
  const bodyMarkup = escTemplate(buildBody(mod, index));
  const missionMarkup = escTemplate(mod.mission);
  const widgetCode = escTemplate(sharedWidgetCode);
  const fileContent = `window.Lessons.lesson4.modules[${index}] = {
    title: "${escDouble(mod.title)}",
